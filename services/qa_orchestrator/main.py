# services/qa_orchestrator/main.py (ЧАСТЬ 1 ИЗ 2)
import os
import io
import glob
...
async def run_stories_by_scope(scope: str):
    ...
    if scope_param == "all":
        ...
        if not os.path.exists(scope_dir):
            raise HTTPException(status_code=404, detail=f"Зона тестирования '{clean_scope}' не найдена.")
        feature_files = sorted(glob.glob(f"{scope_dir}/[0-9][0-9]_*.feature"))

    if not feature_files:
        return Response(content=f"Сценарии в выбранной зоне '{scope_param}' отсутствуют.\n", media_type="text/plain")
    ...
    for feature_path in feature_files:
        ...
        try:
            # Вызываем наш вынесенный изолированный компонент-раннер!
            step_results = await execute_steps_file(steps_files[0])
            failed_steps = [step for step in step_results if "❌" in step]
            
            if failed_steps:
                report["failed"] += 1
                report["errors"].append({"story": filename, "step": failed_steps[0].replace("❌ ", "")})
...
async def generate_rich_response(report, table_rows, string_io, console):
    ...
    for fn, t_color, status in table_rows:
        table.add_row(f"[{t_color}]• {fn}[/{t_color}]", f"[{t_color}]{status}[/{t_color}]")
    console.print(table)
    
    if report["errors"]:
        console.print("\n[bold yellow]🔍 ДЕТАЛИЗАЦИЯ КРИТИЧЕСКИХ ОШИБОК ДЛЯ ОТЛАДКИ:[/bold yellow]")
        for err in report["errors"]:
            console.print(f"[red]• История:[/red] {err['story']}\n  [red]Сбой на шаге:[/red] [bold yellow]{err['step']}[/bold yellow]\n")
    console.print("─"*100 + "\n")
    return Response(content=string_io.getvalue(), media_type="text/plain; charset=utf-8", status_code=400 if report["failed"] > 0 else 200)
