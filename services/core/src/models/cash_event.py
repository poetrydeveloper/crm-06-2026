...
class CashEvent(Base):
    ...
    type: Mapped[CashEventType] = mapped_column(
        Enum(CashEventType), default=CashEventType.SALE, nullable=False
    )
    ...
    amount_card: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), default=Decimal("0.00"), nullable=False
    )
    amount_credit: Mapped[Decimal] = mapped_column(
        Numeric(10, 2), default=Decimal("0.00"), nullable=False
    )  # Уходит в минус баланса клиента

    description: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True
    )  # Примечание/ошибка кассира
    created_by: Mapped[str] = mapped_column(
        String(100), default="admin", nullable=False
    )  # Кто оформил
    ...
    customer: Mapped[Optional["Customer"]] = relationship(
        "Customer", back_populates="cash_events"
    )
    items: Mapped[List["CashEventItem"]] = relationship(
        "CashEventItem", back_populates="cash_event"
    )
