...
class CashEventItem(Base):
    ...
    cash_event_id: Mapped[int] = mapped_column(
        ForeignKey("cash_events.id"), nullable=False
    )

    # Связь с конкретной поштучной единицей товара, ушедшей со склада
    product_unit_id: Mapped[int] = mapped_column(
        ForeignKey("product_units.id"), nullable=False
    )
    ...
    product_unit: Mapped["ProductUnit"] = relationship(
        "ProductUnit", back_populates="cash_event_items"
    )
