# services/core/src/routers/catalog_nodes/search.py
...
async def smart_search(q: str = Query(..., min_length=2), db: AsyncSession = Depends(get_db)):
    ...
    for prod in products:
        ...
        tags = prod.search_tags if isinstance(prod.search_tags, list) else []
        aliases = prod.search_aliases if isinstance(prod.search_aliases, list) else []
