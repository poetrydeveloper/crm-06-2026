    ...
    return [
        {
            ...
            "name": p.name,
            "code": p.code,
            "recommended_retail_price": float(p.recommended_retail_price)
            if p.recommended_retail_price
            else 0.0,
            "category_id": p.category_id,
            "search_tags": p.search_tags if isinstance(p.search_tags, list) else [],
            "search_aliases": p.search_aliases
            if isinstance(p.search_aliases, list)
            else [],
...
async def create_product(payload: ProductCreate, db: AsyncSession = Depends(get_db)):
    ...
    if existing.scalar_one_or_none():
        raise HTTPException(
            status_code=400, detail="Товар с таким артикулом уже зарегистрирован"
        )

    category = await db.get(Category, payload.category_id)
    if not category:
        raise HTTPException(
            status_code=404, detail=f"Категория с ID {payload.category_id} не найдена"
        )

    brand = await db.get(Brand, payload.brand_id)
    if not brand:
        raise HTTPException(
            status_code=404, detail=f"Бренд с ID {payload.brand_id} не найден"
        )

    clean_name = (
        payload.name.lower().replace(",", " ").replace(".", " ").replace("-", " ")
    )
    stop_words = {"и", "в", "на", "под", "с", "по"}
    tags = [
        word for word in clean_name.split() if len(word) > 1 and word not in stop_words
    ]
        ...
        async with httpx.AsyncClient() as client:
            await client.post(
                ...
                json={
                    ...
                    "operation_code": "0003",
                    "level": "INFO",
                    "message": f"Создан товар: {new_product.name} (арт. {new_product.code}, категория: {category.id})",
                },
                timeout=1.0,
            )
    except Exception:
...
async def update_product(
    ...
    linked_units = await db.execute(
        select(ProductUnit).where(ProductUnit.product_id == product_id).limit(1)
    )
    if linked_units.scalar_one_or_none():
        raise HTTPException(
            status_code=400, detail="Нельзя редактировать товар с созданными единицами"
        )
...
async def delete_product(product_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if linked_units.scalar_one_or_none():
        raise HTTPException(
            status_code=400,
            detail="Нельзя удалить товар с созданными единицами на складе",
        )
