...
from src.models import Product, ProductUnit, Brand
...
async def get_units_by_category(category_id: int, db: AsyncSession = Depends(get_db)):
    """📦 Возвращает юниты в категории с остатками, ожиданием и брендом"""
    products = (
    ...
    for product in products:
        ...
        # Количество EXPECTED (в заявке, не пришли)
        expected_qty = (
            await db.execute(
                select(func.count(ProductUnit.id)).where(
                    ProductUnit.product_id == product.id,
        ...
        units = (
            (
                await db.execute(
                    ...
                    .where(
                        ProductUnit.product_id == product.id,
                        ProductUnit.physical_status.in_(
                            ["IN_STORE", "IN_DISASSEMBLED"]
                        ),
                    )
        ...
        result.append(
            {
                ...
                "recommended_retail_price": float(
                    product.recommended_retail_price or 0
                ),
