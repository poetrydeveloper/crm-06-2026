...
# =========================================================================
# 📦 КЛИЕНТСКИЕ СХЕМЫ ДЛЯ ОБРАТНОЙ СБОРКИ НАБОРА (СТАДИЯ 15)
# =========================================================================
...
# =========================================================================
# 🛑 КОМАНДА 0302: КОМПЛЕКТАЦИЯ НАБОРА ИЗ ОДИНОЧНЫХ САТЕЛЛИТОВ
# =========================================================================
...
async def process_set_absorption(payload: SetAbsorptionPayload, db: AsyncSession = Depends(get_db)):
    """
    Принимает список ID штучных деталей, переводит их в ABSORBED, блокирует для кассы,
    и рождает 1 готовый целый родительский набор инструментов на складе (Код 0302).
    """
    # 1. Проверяем существование номенклатурной карточки набора
    parent_product = await db.get(Product, payload.parent_product_id)
    if not parent_product:
        raise HTTPException(status_code=404, detail=f"Карточка набора с ID {payload.parent_product_id} не найдена")

    if not payload.satellite_unit_ids:
        raise HTTPException(status_code=400, detail="Список сателлитов для сборки не может быть пустым")

    # 2. Извлекаем и валидируем все сателлиты из базы данных с блокировкой строки
    result = await db.execute(
        select(ProductUnit).where(ProductUnit.id.in_(payload.satellite_unit_ids)).with_for_update()
    )
    satellites = result.scalars().all()

    if len(satellites) != len(payload.satellite_unit_ids):
        raise HTTPException(status_code=404, detail="Один или несколько указанных сателлитов не найдены в СУБД")

    # Считаем общую себестоимость набора на основе закупки входящих в него деталей
    ...
    # 3. Генерируем новый родительский юнит целого набора инструментов с простановкой таймстампа
    generated_parent_sn = f"SN-SET-{uuid.uuid4().hex[:8].upper()}"
    parent_unit = ProductUnit(
        product_id=payload.parent_product_id,
    ...
    # 4. Переводим сателлиты в состояние ABSORBED и привязываем parent_unit_id
    for sat in satellites:
    ...
    # 5. АСИНХРОННОЕ МЕЖСЕРВИСНОЕ ЛОГИРОВАНИЕ (Код 0302)
    async with httpx.AsyncClient() as client:
        try:
            await client.post(
                "http://logger:8001/api/v1/log", 
                json={
                    ...
                    "operation_code": "0302", 
                    "level": "INFO", 
                    "message": f"Комплектация набора {parent_product.name} (SN: {generated_parent_sn}) завершена. Поглощено деталей: {len(satellites)} шт. Сателлиты переведены в ABSORBED."
                },
                timeout=1.0
            )
        except Exception:
