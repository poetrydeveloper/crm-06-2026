...
class ReceiptManager:
    @staticmethod
    async def process_receipt_by_units(supplier_id: int, unit_ids: List[int], db: AsyncSession) -> dict:
        ...
        for unit_id in unit_ids:
            ...
            if unit.physical_status != PhysicalStatus.EXPECTED:
                errors.append(f"Юнит #{unit_id} уже имеет статус {unit.physical_status.value}")
                continue
            ...
            accepted_items.append({
                ...
                "product_name": product.name if product else f"Товар #{unit.product_id}",
        ...
        return {
            "status": "success",
            "message": f"Принято: {accepted_count} шт." + (f" (ошибки: {len(errors)})" if errors else ""),
